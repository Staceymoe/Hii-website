...
- Before creating or adding a visual, search the designated Hii website asset library for an appropriate approved asset. Rename every selected asset immediately using section, subject, asset type, and version. Filenames must explicitly distinguish `candidate`, `approved`, `concept`, and `final`; storage location alone never establishes approval.
